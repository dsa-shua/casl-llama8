...
# Start Profiling
with profile(activities=[ProfilerActivity.CUDA], record_shapes=True, profile_memory=True, with_stack=True, with_flops=True, with_modules=True) as prof:
   with record_function("model_inference"):
       res = pipeline(requests[:BATCH_SIZE], batch_size=BATCH_SIZE, max_new_tokens=OUT_LEN, truncation="longest_first")


end_time = time.time()

# # Save logs
with open(f"/root/casl/llama/results/B{BATCH_SIZE}/O{OUT_LEN}/I{INPUT_LEN}/all.txt" , "w") as outfile:
    outfile.write(prof.key_averages().table(sort_by="cuda_time_total", max_name_column_width=90))

# # Show a few on console    
